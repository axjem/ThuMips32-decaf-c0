...
import decaf.error.FloatFormatError;
...
public abstract class BaseLexer {
	...
	protected int charConst(String sval, Location loc) {
		setSemantic(loc, SemValue.createLiteral(Tree.CHAR, sval));
		return Parser.LITERAL;
	}
	
	protected int floatConst(String fval) {
		try {
			setSemantic(getLocation(), SemValue.createLiteral(
					Tree.FLOAT, Float.valueOf(Float.parseFloat(fval))));
		} catch (NumberFormatException e) {
			Driver.getDriver().issueError(
					new FloatFormatError(getLocation(), fval));
		}
		return Parser.LITERAL;
	}
